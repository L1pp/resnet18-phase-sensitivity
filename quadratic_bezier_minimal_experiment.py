...
PROFILES: Dict[str, Profile] = {
    ...
    "minimal": Profile("minimal", 2000, 256, 512, 20, 64, 64, 16, 4),
}
...
def config(profile: str | Profile) -> Dict[str, Any]:
    ...
    return {
        ...
        "optimizer": {"name": "AdamW", "lr": 1e-3, "weight_decay": 1e-4, "amp": True, "scheduler": "ReduceLROnPlateau(factor=0.5,patience=3)"},
...
def fingerprint(profile: str | Profile) -> str:
    text = json.dumps(config(profile), sort_keys=True, separators=(",", ":"))
...
class OfflineDataset(Dataset[Tuple[torch.Tensor, torch.Tensor]]):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor]:
        x = torch.from_numpy(self.images[index].copy()).unsqueeze(0).repeat(3, 1, 1).float().div_(255.0)
        return x, torch.from_numpy(self.targets[index]).float()
...
def train(profile: str = "minimal", force: bool = False) -> Dict[str, Any]:
    ...
    model = build_model().to(dev); optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4); scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", factor=.5, patience=3, min_lr=1e-5); loss_fn = nn.SmoothL1Loss(beta=0.02); scaler = torch.amp.GradScaler("cuda", enabled=amp)
    loader = DataLoader(OfflineDataset(train_images, train_targets), batch_size=spec.batch_size, shuffle=True, num_workers=0, generator=torch.Generator().manual_seed(SEED))
    history: List[Dict[str, Any]] = []; best_val = math.inf
    for epoch in range(1, spec.epochs + 1):
        ...
        with torch.no_grad():
            for x, y in DataLoader(OfflineDataset(val_images, val_targets), batch_size=spec.batch_size, num_workers=0):
                with _amp_context(dev, False): pred = model(x.to(dev)); val_loss += float(loss_fn(pred, y.to(dev))) * len(x); val_out.append(pred.cpu().numpy())
        val_loss /= max(1, len(val_targets)); train_loss = total / max(1, count); val_pred = np.concatenate(val_out); val_mae_px = float(np.mean(np.abs(val_pred - val_targets)) * 223)
        lr_used = float(optimizer.param_groups[0]["lr"]); scheduler.step(val_loss); lr_next = float(optimizer.param_groups[0]["lr"])
        ...
        if val_loss < best_val:
            best_val = val_loss; payload = {"model_state": model.state_dict(), "epoch": epoch, "fingerprint": fingerprint(spec), "profile": spec.name}; d["checkpoints"].mkdir(parents=True, exist_ok=True); torch.save(payload, best)
    torch.save({"model_state": model.state_dict(), "epoch": spec.epochs, "fingerprint": fingerprint(spec), "profile": spec.name}, d["checkpoints"] / "last.pt")
    write_rows(d["tables"] / "history.csv", history); _write_metadata(spec, "train", {"best_val_loss": best_val})
...
def _plot_analysis(profile: str | Profile, images: np.ndarray, ids: np.ndarray, targets: np.ndarray, predictions: np.ndarray, row: Mapping[str, Any]) -> None:
    ...
    ax.set_xlabel("epoch"); ax.set_ylabel("SmoothL1"); ax.set_title("loss and validation MAE"); ax.grid(alpha=.2); ax.legend(loc="upper left")
...
def summarize(profile: str | Profile = "minimal", force: bool = False) -> Dict[str, Any]:
    ...
    result = {
        ...
        "last_epoch": spec.epochs,
        "best_validation_at_last_epoch": int(best_history["epoch"]) == spec.epochs,
        "network": network,
    ...
    flat = {"profile": spec.name, "fingerprint": fingerprint(spec), "best_validation_epoch": result["best_validation_epoch"], "last_epoch": spec.epochs, "best_validation_at_last_epoch": result["best_validation_at_last_epoch"], **{f"network_{k}": v for k, v in network.items()}, **{f"jacobian_{k}": v for k, v in identifiability_result.items()}, **{f"inverse_{k}": v for k, v in inverse_result.items()}}
